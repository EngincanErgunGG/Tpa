...
class Tpa extends PluginBase implements Listener
{
    ...
    public function onCommand(CommandSender $e, Command $kmt, string $lbl, array $args): bool
    {
        ...
        if ($kmt->getName() == "tpak")
        {
            if ($e instanceof Player)
            {
                ...
                {
                    $o = $this->getServer()->getPlayerExact($this->istek[$e->getName()]);
                    if ($o instanceof Player)
                    {
                        $o->teleport($e->getLocation());
    ...
    public function tpa($e)
    {
        $f = new CustomForm(function (Player $e, array $data = null)
        {
            ...
            if (!$data[0] == null)
            {
                if ($o = $this->getServer()->getPlayerExact($data[0]))
                {
                    ...
                    {
                        if ($o->getWorld()->getFolderName() == $worlds)
                        {
                            $e->sendMessage("§cBu oyuncu, şuanda §6" . $o->getWorld()->getFolderName() . " §cadlı dünyada. Buraya gidilmesi yasaktır.");
                            return false;
                        }
                    }
                    if ($o->getName() == $e->getName())
